        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-6 col-12">
                    <div class="ecom-wc__form">
                        <div class="ecom-wc__form-inner">
                            <h3 class="ecom-wc__form-title ecom-wc__form-title__one">{{__('user.Login')}}
                                <span>{{__('user.To access our pltafrom please login first')}}</span></h3>
                            <!-- Sign in Form -->
                            <form class="ecom-wc__form-main p-0" action="{{ route('store-login') }}" method="post">
                                ...
                                <div class="form-group homec-form-input">
                                    ...
                                    <div class="form-group__input">
                                        <input class="ecom-wc__form-input" id="password-field" type="password"
                                            name="password" placeholder="{{__('user.Password')}}">
                                    </div>
                                </div>

                                @if($recaptcha_setting->status==0)
                                <div class="form-group homec-form-input">
                                ...
                                <div class="form-group form-mg-top-30">
                                    <div class="ecom-wc__button ecom-wc__button--bottom">
                                        <button class="homec-btn homec-btn__second"
                                            type="submit"><span>{{__('user.Login')}}</span></button>

                                        @if ($social_login->is_gmail == 1)
                                        <button id="googleLoginBtn"
                                            class="homec-btn homec-btn__second homec-btn__social" type="button">
                                            <div class="ntfmax-wc__btn-icon"><img
                                                    src="{{ asset($path.'frontend/img/google.svg') }}"></div>
                                            <span>{{__('user.Sign in with Google')}}</span>
                                        </button>
                                        @endif
                                ...
                                <div class="form-group mg-top-20">
                                    <div class="ecom-wc__bottom">
                                        <p class="ecom-wc__text">{{__('user.Do not have an account ?')}} <a
                ...
                <div class="col-lg-6 col-12 d-none d-lg-block ">
                    <div class="ecom-wc__inner homec-bg-cover homec-welcome-bg"
                        style="background-image: url({{ asset($login_page->login_bg_image) }});">
                        <!-- Logo -->
                        <div class="ecom-wc__logo">
                            <a href="{{ route('home') }}"><img src="{{ asset($login_page->login_page_logo) }}"
                                    alt="image"></a>
                        </div>
                        <div class="ecom-wc__inside">
                            <!-- Middle Image -->
                            <div class="ecom-wc__middle">
                                <a href="{{ route('home') }}"><img src="{{ asset($login_page->image) }}"
                                        alt="image"></a>
                                <div class="ecom-wc__countdown--title">{{ $login_page->login_top_item_qty }}<span>{{
                                        $login_page->login_top_item }}</span></div>
                                <div class="ecom-wc__countdown--title ecom-wc__countdown--title--v2">{{
                                    $login_page->login_footer_item_qty }}<span>{{ $login_page->login_footer_item
                                        }}</span></div>
                            </div>
